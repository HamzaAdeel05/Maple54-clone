...
const FutureSearch = () => {
    ...
    return (
        <div className='bg-[#F6F0E6] py-10 my-10'>
            <div className="flex justify-center items-center flex-col gap-5 py-20 px-25 space-y-4">
                <h1 className='font-bold text-4xl text-[#431616]'>The Future of Search Engine Optimization</h1>
                ...
                <h4 className='font-bold text-lg'>Search engines have evolved.</h4>
                <p className='text-2xl font-semibold'>They no longer just match keywords; they understand the meaning behind your content.Semantic
                    ...
                    <div className="grid grid-cols-4 gap-10">
                        <div className="">
                            {seoServices.map((item: any) => (
                                <div onClick={() => setSelected(item)} key={item.key} className={cn("px-6 hover:cursor-pointer border-b border-dotted py-4 ",selected===item ? "bg-[#431616] ":"")}>
                                    <h2 className={cn('font-medium text-lg hover:text-[#FA5131] text-gray-700',selected===item ? "text-[#F6F0E6]": "")}>{item.page_title}</h2>
                                </div>
                            ))}
                        </div>
