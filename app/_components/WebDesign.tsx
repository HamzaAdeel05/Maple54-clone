...
export default function ServicesGrid() {
 ...
 const services = [
    ...
    // ⭐ MERGED BIG CARD (Dynamic) - ID skipped
    {
        merged: true,
    },
                    ...
                    <div key={card.id || "merged"} className={card.merged ? "row-span-3" : " "}>
                        {card.merged ? (
                            ...
                            <Card onClick={() => setSelected(card)} className={cn(
                                ...
                                selected.title === card.title ? " text-white bg-[#3F5C7C]" : "bg-white text-black"
                            )}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={cn("h-10 w-10 bg-[#3F5C7C] text-white flex justify-center items-center rounded-full", selected.title === card.title ? "bg-white text-[#3F5C7C]" : "")}>
                                        {card.icon}
                                    </div>
                                    <h2 className={cn("font-bold text-2xl text-[#431616]", selected.title === card.title ? "text-white" : "")}>{card.title}</h2>
                                </div>
                                <p className={cn("font-semibold text-gray-800", selected.title === card.title ? "text-white" : "")}>{card.desc}</p>
                                <div className="flex justify-end  ">
                                    <div className={cn("border-2 border-[#3F5C7C] rounded-full h-8 w-8 flex justify-center items-center", selected.title === card.title ? "bg-gray-200" : "")}>
