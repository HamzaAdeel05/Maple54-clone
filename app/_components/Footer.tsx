    ...
    return (
        <div>
            <div className=" bg-[#431616] text-white">
                <div className="grid grid-cols-2 xl:px-5 px-4 gap-4 xl:flex py-10 xl:py-20 justify-evenly">
                    <div className="xl:max-w-75 space-y-10">
                        <div className="space-y-4">
                            <img src="https://www.maple54.com/_next/static/media/MapleSmallLogo.45f8fb3a.svg" alt="" />
                            <p className='font-medium xl:text-xl text-[14px]'>Know about Digital Marketing Trends
                                Sign up for our newsletter:</p>
                            <Input className='placeholder:text-white placeholder:font-medium placeholder:text-lg xl:px-6 py-5' placeholder='Email Address' />
                        </div>
                        <div className="mt-5 space-y-4">
                            <h2 className='font-bold xl:text-lg'>24/7 Support:</h2>
                            <div className="flex gap-3  items-center">
                                <Phone size={20}  />
                                <p className='font-medium xl:text-lg'>  +1 (480) 650-9911</p>
                            </div>
                            <div className="flex gap-3  items-center">
                                <Mail size={20} />
                                <p className='font-medium xl:text-lg'>info@maple54.com</p>
                            </div>
                        </div>
                        <div className="space-y-3">
                            ...
                            <div className="flex gap-2 xl:gap-3">
                                {socialIcons.map((logos) => (
                                    <div key={logos.name} className="xl:w-10 w-7 rounded-full">
                    ...
                    <div className="xl:block md:hidden mt-5 px-4">
                        <div className="space-y-2 w-fit">
                            {Headings.map((heading) => (
                                <div key={heading} className="border border-white rounded-lg  px-5 py-1 xl:py-2 flex ">
                                    <p className='font-bold xl:text-lg'>{heading}</p>
                                </div>
                            ))}
                        </div>
                        <h2 className='xl:text-lg font-bold mt-10 mb-5'>Products</h2>
                        <div className="space-y-3">
                            {Products.map((p) => (
                                <p className='xl:text-lg font-medium text-[15px]' key={p}>{p}</p>
                            ))}
                        </div>
                    </div>
                    <div className="md:grid hidden  xl:w-200  ">
                        <div className="grid grid-cols-2 rounded-2xl ">
                            <div className="">
                                {CategoriesArray.map((c) => (
                                    <div key={c.id} className="border border-white md:px-6  xl:px-10 md:py-3 xl:py-4">
                                        <p className='font-semibold  md:text-[15px]'>{c.name}</p>
                                    </div>
                                ))}
                            </div>
                            <div className="w-full">
                                {LinksArray.map((l) => (
                                    <div key={l.id} className="border border-white p-2 ">
                                        <p className='md:text-[12px] xl:text-sm font-medium '>{l.name}</p>
                                    </div>
                                ))}
                            </div>
                ...
                <h2 className='text-center md:mb-4 py-2 md:py-6 text-3xl md:text-6xl xl:text-[10rem] font-bold border-t'>Build with Maple 54</h2>
                <div className="flex flex-col justify-center items-center">
                    <div className="flex border-y w-full justify-center gap-6 p-4">
                        {PoliciesArray.map((p)=>(
                            <div key={p.id} className="flex gap-4  ">

                            <h2 className='font-medium text-[12px] text-center'>{p.name}</h2>
                            <p className=''>{p.add}</p>
                        </div>
                        ))}
                    </div>
                    <h2 className='font-medium text-sm xl:text-lg my-5'>Copyright © 2025 Maple 54, LLC. All rights reserved.</h2>
                    <img className='xl:w-200 md:w-100 w-70 mb-3' src="https://www.maple54.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FvenorsImg.68515df8.jpg&w=1920&q=75" />
                </div>
            </div>
        </div>
    )
